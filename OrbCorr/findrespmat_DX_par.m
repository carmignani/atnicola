function C = findrespmat_DX_par(RING, indbpm, indquad, PLANE, PVALUE, ORBITFUNCTION)
...
%         if isfield(ring_p{i}{indquad(i)},'T1')
%             T1 = getfield(ring_p{i}{indquad(i)},'T1');
%             oldvalue=T1(indplane);
%         else
%             oldvalue=0;
%         end
        if PLANE == 'H'
            ring_p{i}=atsetshift(ring_p{i},indquad(i),+PVALUE(i),0,'RelativeShift');
        elseif PLANE == 'V'
            ring_p{i}=atsetshift(ring_p{i},indquad(i),0,+PVALUE(i),'RelativeShift');
        end
        ORBITPLUS  = feval(orbit_function_handle,ring_p{i},orbit_function_args{:});
        if PLANE == 'H'
            ring_p{i}=atsetshift(ring_p{i},indquad(i),-2*PVALUE(i),0,'RelativeShift');
        elseif PLANE == 'V'
            ring_p{i}=atsetshift(ring_p{i},indquad(i),0,-2*PVALUE(i),'RelativeShift');
        end
        ORBITMINUS  = feval(orbit_function_handle,ring_p{i},orbit_function_args{:});
        if PLANE == 'H'
            ring_p{i} = atsetshift(ring_p{i},indquad(i),+PVALUE(i),0,'RelativeShift');
        elseif PLANE == 'V'
            ring_p{i}=atsetshift(ring_p{i},indquad(i),0,+PVALUE(i),'RelativeShift');
